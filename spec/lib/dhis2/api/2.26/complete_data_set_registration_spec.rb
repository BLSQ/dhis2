...
describe Dhis2::Api::Version226::CompleteDataSetRegistration do
  let(:client) { build_client("2.26") }
  let(:get_response) {
    [Dhis2::Api::Version226::CompleteDataSetRegistration.new(
      client,
      {
        period: "201701",
  ...
  it "list complete data set registrations" do
    stub_request(:get, "https://play.dhis2.org/demo/api/completeDataSetRegistrations?dataSet=HllvX50cXC0&dataSet=HllvX50cXC1&orgUnit=Tht0fnjagHi&orgUnit=VrDA0Hn4Xc6&period=201701&period=201702")
      .to_return(status: 200, body: fixture_content("2.26", "list", "complete_data_set_registrations.json"))
